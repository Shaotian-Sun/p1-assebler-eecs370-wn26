...
void offsetError(int);
void registerError(char *);
...
int main(int argc, char **argv)
{
    ...
    char labelTable[MAXLINELENGTH][MAXLINELENGTH];
    ...
    {
        strcpy(labelTable[line], label);

        /* error: duplicate labels*/
        for (int i = 0; i < line; i++)
        {
            if (label[0] != '\0' && !strcmp(labelTable[i], label))
            {
    ...
    while (readAndParse(inFilePtr, label, opcode, arg0, arg1, arg2))
    {
        if (!strcmp(opcode, "add"))
        {
            ...
            registerError(arg1);
            registerError(arg2);
            mc = (0 << 22) + (atoi(arg0) << 19) + (atoi(arg1) << 16) + atoi(arg2);
        ...
        else if (!strcmp(opcode, "nor"))
        {
            ...
            registerError(arg1);
            registerError(arg2);
            mc = (1 << 22) + (atoi(arg0) << 19) + (atoi(arg1) << 16) + atoi(arg2);
            printHexToFile(outFilePtr, mc);
        }
        else if (!strcmp(opcode, "lw"))
        {
            ...
            offsetError(atoi(arg2));
            mc = (2 << 22) + (atoi(arg0) << 19) + (atoi(arg1) << 16) + (atoi(arg2) & 0xFFFF);
            printHexToFile(outFilePtr, mc);
        }
        else if (!strcmp(opcode, "sw"))
        {
            ...
            offsetError(atoi(arg2));
            mc = (3 << 22) + (atoi(arg0) << 19) + (atoi(arg1) << 16) + (atoi(arg2) & 0xFFFF);
            printHexToFile(outFilePtr, mc);
        }
        else if (!strcmp(opcode, "beq"))
        {
            ...
            offsetError(atoi(arg2));
            mc = (4 << 22) + (atoi(arg0) << 19) + (atoi(arg1) << 16) + (atoi(arg2) & 0xFFFF);
            printHexToFile(outFilePtr, mc);
        }
...
// Exit 1 if offset exceeds 16 bit
void offsetError(int offset)
{
...
// Exit 1 if register non-integer or outside [0,7]
void registerError(char *reg)
{
    if (!isNumber(reg))
    {
        printf("register non-integer %s\n", reg);
        exit(1);
    }
    else if (atoi(reg) < 0 || atoi(reg) > 7)
    {
        printf("register outside [0,7] %s\n", reg);
        exit(1);
    }
}